...
class GeminiWebLLM:
    ...
    def generate_json(self, prompt: str) -> any:
        ...
        if json_match:
            ...
            # Fallback: Extract everything between the first { or [ and the last } or ]
            start_idx = response_text.find('{')
            start_arr = response_text.find('[')
            
            if start_idx != -1 and (start_arr == -1 or start_idx < start_arr):
                end_idx = response_text.rfind('}')
                if end_idx != -1:
                    raw_json = response_text[start_idx:end_idx+1]
                else:
                    raw_json = response_text.strip()
            elif start_arr != -1:
                end_idx = response_text.rfind(']')
                if end_idx != -1:
                    raw_json = response_text[start_arr:end_idx+1]
                else:
                    raw_json = response_text.strip()
            else:
                raw_json = response_text.strip()
        ...
        try:
            ...
            print(f"[GeminiWebLLM] Raw Extracted String: {raw_json[:100]}...")
            return None
