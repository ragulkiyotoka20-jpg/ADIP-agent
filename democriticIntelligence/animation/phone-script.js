...
const PHASE_START_2 = 60;  // 3D Tilt & Glass Bubbles Expansion (2s - 4.5s)
const PHASE_START_3 = 135; // Straight Back & Cursor Click (4.5s - 6.5s)
const PHASE_START_4 = 195; // Close-up Card Zoom & Confetti (6.5s - 8s)
...
const confettiSeeds = [];
const confettiColors = ['#f43f5e', '#3b82f6', '#10b981', '#f59e0b', '#a855f7', '#ec4899', '#06b6d4'];
...
function updateShowcase(frame) {
  ...
  if (frame >= PHASE_START_1 && frame < PHASE_START_2) {
    ...
    const entryProg = Math.min(1, frame / 20); // Quick initial entry if frame restarts
  ...
  // -------------------------------------------------------------
  // PHASE 2: 3D Tilt & Glass Bubbles Expansion (Frames 60 to 135)
  // -------------------------------------------------------------
  ...
  // -------------------------------------------------------------
  // PHASE 4: Close-up Card Zoom & Confetti (Frames 195 to 240)
  // -------------------------------------------------------------
  if (frame >= PHASE_START_4) {
    ...
    // Zoom in on Card 1 (scale 1.6x, center it)
    // To center Card 1: translate the wrapper by (-60px, -100px) and scale 1.65x
