...
def main():
    print("==================================================")
    print(" LAUNCHING SIGNED-IN GEMINI WEB (VISIBLE CHROMIUM)")
    print("==================================================\n")
    
    # Path to signed-in user data profile
    profile_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "universal-director-agent", "democriticIntelligence", "gemini_user_data"))
    
    # Launch visible Chromium using signed-in account profile
    llm = GeminiWebLLM(user_data_dir=profile_dir, headless=False)
    
    prompt = """Write a MASSIVE, 1000+ LINE STANDALONE SINGLE-FILE HTML (with complete embedded CSS and JS) for a 25-second 3D product showcase video about "Royal Challengers Bengaluru (RCB) IPL 2026 Season Launch".
...
Return ONLY the complete HTML code starting with <!DOCTYPE html>. Do NOT wrap in markdown backticks or explanations."""
    ...
    print("-> Opening visible Chromium with your signed-in Gemini account...")
    response = llm.ask_gemini(prompt, timeout=120)
    
    if not response:
        print("[FAIL] Web Gemini response was empty. Retrying with explicit prompt...")
        return
        
    print(f"-> Received response from Signed-In Web Gemini ({len(response)} chars)!")
    ...
    output_html = os.path.abspath("signed_gemini_rcb.html")
    with open(output_html, "w", encoding="utf-8") as f:
        f.write(html_code)
        
    print(f"\n==================================================")
    print(f" SUCCESS: SIGNED-IN GEMINI CODE GENERATED AT:")
    print(f" {output_html}")
